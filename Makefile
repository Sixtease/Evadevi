...
hmms/3-aligned/hmmdefs hmms/3-aligned/macros data/transcription/train/aligned.mlf: hmms/2-sp/hmmdefs hmms/2-sp/macros data/transcription/train/trans.mlf $(reest_prereq) data/wordlist/WORDLIST-train-sil-phonet data/phones/monophones
...
hmms/1-init/hmmdefs hmms/1-init/macros: resources/hmm/proto data/phones/monophones-nosp $(reest_prereq) data/transcription/train/phonetic-nosp.mlf data/wordlist/WORDLIST-test-unk-nosp-phonet $(EV_LM) data/transcription/heldout.mlf $(EV_train_mfcc)
	...
	hmmeval.pl --hmmdir hmms/1-init --workdir temp/test --phones hmms/1-init/phones --conf resources/htk-config --wordlist data/wordlist/WORDLIST-test-unk-nosp-phonet --LM "$(EV_LM)" --trans data/transcription/heldout.mlf --mfccdir "$(EV_train_mfcc)"

data/transcription/train/phonetic-nosp.mlf: $(EV_wordlist_train_phonet) resources/mkphones0.led data/transcription/train/trans.mlf
	mkdir -p data/transcription/train temp
	LANG=C H HLEd -l '*' -d $(EV_wordlist_train_phonet) -i temp/phonetic-nosp-missing-sil.mlf resources/mkphones0.led data/transcription/train/trans.mlf
...
data/transcription/heldout.mlf data/transcription/train/trans.mlf: $(EV_train_transcription)
	mkdir -p data/transcription/train
	split-mlf.pl "$(EV_train_transcription)" data/transcription/train/trans.mlf=$(train_heldout_ratio) data/transcription/heldout.mlf=1
