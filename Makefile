...
test: hmms/5-mixtures/hmmdefs hmms/5-mixtures/macros hmms/5-mixtures/phones data/wordlist/WORDLIST-test-unk-phonet $(EV_LM) $(EV_test_transcription) $(EV_test_mfcc)
	mkdir -p temp/test
	bin/hmmeval.pl --hmmdir hmms/5-mixtures --workdir temp/test --phones hmms/5-mixtures/phones --conf resources/htk-config --wordlist data/wordlist/WORDLIST-test-unk-phonet --LM "$(EV_LM)" --trans "$(EV_test_transcription)" --mfccdir "$(EV_test_mfcc)" -t '100.0'
...
hmms/5-mixtures/hmmdefs hmms/5-mixtures/macros: $(model_to_add_mixtures_to)/hmmdefs $(model_to_add_mixtures_to)/macros $(mixture_phones) $(reest_prereq) data/transcription/train/aligned.mlf $(EV_monophones) data/wordlist/WORDLIST-test-unk-phonet
	...
	EV_HVite_s=5.0 EV_HVite_p=0.0 bin/add-mixtures.pl -a $(mixture_opt)
...
hmms/3-aligned/hmmdefs hmms/3-aligned/macros data/transcription/train/aligned.mlf: hmms/2-sp/hmmdefs hmms/2-sp/macros $(EV_train_transcription) $(reest_prereq) bin/generate-scp.pl data/wordlist/WORDLIST-train-sil-phonet bin/remove-empty-sentences-from-mlf.pl $(EV_monophones)
	...
	LANG=C H HVite -T 1 -A -D -l '*' -C resources/htk-config -t "$(EV_HVite_t)" -H hmms/2-sp/macros -H hmms/2-sp/hmmdefs -S temp/train-mfc.scp -i temp/trancription-aligned-with-empty.mlf -m -I "$(EV_train_transcription)" -y lab -a -o SWT -b silence data/wordlist/WORDLIST-train-sil-phonet "$(EV_monophones)"
	bin/remove-empty-sentences-from-mlf.pl < temp/trancription-aligned-with-empty.mlf > data/transcription/train/aligned.mlf
	bin/hmmiter.pl --iter 3 --indir hmms/2-sp --outdir hmms/3-aligned --workdir hmms/3-aligned/iterations --mfccdir "$(EV_train_mfcc)" --conf resources/htk-config --mlf data/transcription/train/aligned.mlf --phones "$(EV_monophones)"
	bin/hmmeval.pl --hmmdir hmms/3-aligned --workdir temp/test --phones hmms/3-aligned/phones --conf resources/htk-config --wordlist data/wordlist/WORDLIST-test-unk-phonet --LM "$(EV_LM)" --trans "$(EV_heldout_transcription)" --mfccdir "$(EV_heldout_mfcc)"

hmms/2-sp/hmmdefs hmms/2-sp/macros: bin/add-sp.pl hmms/1-init/hmmdefs hmms/1-init/macros resources/sil.hed $(EV_monophones) $(reest_prereq) data/transcription/train/phonetic-nosp.mlf data/wordlist/WORDLIST-test-unk-phonet $(EV_LM) $(EV_heldout_transcription) $(EV_heldout_mfcc)
	...
	bin/hmmiter.pl --iter 3 --indir hmms/2-sp/base2-sp-sil-tied --outdir hmms/2-sp --workdir hmms/2-sp/iterations --conf resources/htk-config --mfccdir "$(EV_train_mfcc)" --mlf "data/transcription/train/phonetic-nosp.mlf" --phones "$(EV_monophones)"
	bin/hmmeval.pl --hmmdir hmms/2-sp --workdir temp/test --phones hmms/2-sp/phones --conf resources/htk-config --wordlist data/wordlist/WORDLIST-test-unk-phonet --LM "$(EV_LM)" --trans "$(EV_heldout_transcription)" --mfccdir "$(EV_heldout_mfcc)"

hmms/1-init/hmmdefs hmms/1-init/macros: bin/init-hmm.pl resources/hmm/proto data/phones/monophones-nosp $(reest_prereq) data/transcription/train/phonetic-nosp.mlf data/wordlist/WORDLIST-test-unk-nosp-phonet $(EV_LM) $(EV_heldout_transcription) $(EV_heldout_mfcc)
	...
	bin/hmmeval.pl --hmmdir hmms/1-init --workdir temp/test --phones hmms/1-init/phones --conf resources/htk-config --wordlist data/wordlist/WORDLIST-test-unk-nosp-phonet --LM "$(EV_LM)" --trans "$(EV_heldout_transcription)" --mfccdir "$(EV_heldout_mfcc)"
